...
char * typestring(IDENTIFIER_TYPE type)
{
	switch (type)
	{
	case IT_VARIABLE:
...
{
	if (child == NULL)
		fprintf(stderr, "Cannot add NULL child to syntax tree!\n");
...
void TreeNode::ReplaceChild(TreeNode * replaceThis, TreeNode * withThis)
{
	std::list<TreeNode*>::iterator iter;
	for (iter = children.begin(); iter != children.end(); iter++)
	{
	...
	if (firstChild == replaceThis)
		firstChild = withThis;
	else if (secondChild == replaceThis)
		secondChild = withThis;
	else if (thirdChild == replaceThis)
		thirdChild = withThis;
}
...
DeclarationTreeNode::DeclarationTreeNode(IDENTIFIER_TYPE vartype, VariableTreeNode * variable) : TreeNode(NT_DECLARATION)
{
...
ProcDefTreeNode::ProcDefTreeNode(SymbolTable::Entry* symentry, int localScope, ParamsTreeNode * params, BlockTreeNode* block) : TreeNode(NT_PROCDEF)
{
	this->symentry = symentry;
...
void ProcDefTreeNode::PrintNodeAttributes(FILE* out)
{
	fprintf(out, "%s", symentry->Lexeme().c_str());
}

ParamsTreeNode::ParamsTreeNode() : TreeNode(NT_PARAMS)
{
	this->symentry = NULL;
}

ReturnTreeNode::ReturnTreeNode() : TreeNode(NT_RETURN)
{
}

ReturnTreeNode::ReturnTreeNode(TreeNode* returnValue) : TreeNode(NT_RETURN)
{
	AddChild(returnValue);
}

ProcCallTreeNode::ProcCallTreeNode(SymbolTable::Entry* symentry, ArgsTreeNode* args, bool expectReturnValue) : TreeNode(NT_PROCCALL)
{
	this->symentry = symentry;
	AddChild(args);
	this->expectReturnValue = expectReturnValue;
}

void ProcCallTreeNode::PrintNodeAttributes(FILE* out)
{
	fprintf(out, "%s", symentry->Lexeme().c_str());
}

ArgsTreeNode::ArgsTreeNode() : TreeNode(NT_ARGS)
{
}
